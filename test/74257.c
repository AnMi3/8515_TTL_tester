...
#define P_VCC	BIT_H0
	...
	Direct_L = P_S | P_A1 | P_A2 | P_B1 | P_B2 | P_GND;
	Direct_H = P_A3 | P_A4 | P_B3 | P_B4 | P_nEN | P_VCC;

	/* set Pull-up on Y inputs and set HIGH to VCC pin */
	Port_L = P_Y1 | P_Y2;
	Port_H = P_Y3 | P_Y4 | P_VCC;
	delay1ms();
	/* At first it should be only VCC at High state */
	...
	delay1ms();
	/* A, Y and VCC should be High (according to Truth table). nEn, S and B should be Low */
	res &= ( (Pin_L == (P_A1 | P_A2 | P_Y1 | P_Y2)) && (Pin_H == (P_A3 | P_A4 | P_Y3 | P_Y4 | P_VCC)) );

	/* set A High, B Low, nEN Low, S High, Pull-up on Y and High to VCC */
	Port_L = P_S | P_A1 | P_A2 | P_Y1 | P_Y2;
	delay1ms();
	/* S, A and VCC should be High. Y, nEn and B should be Low */
	res &= ( (Pin_L == (P_S | P_A1 | P_A2)) && (Pin_H == (P_A3 | P_A4 | P_VCC)) );

	/* set A Low, B High, nEN Low, S High, Pull-up on Y and High to VCC */
	Port_L = P_S | P_B1 | P_B2 | P_Y1 | P_Y2;
	Port_H = P_B3 | P_B4 | P_Y3 | P_Y4 | P_VCC;
	delay1ms();
	/* S, B, Y and VCC should be High */
	res &= ( (Pin_L == (P_S | P_B1 | P_B2 | P_Y1 | P_Y2)) && (Pin_H == (P_B3 | P_B4 | P_Y3 | P_Y4 | P_VCC)) );
